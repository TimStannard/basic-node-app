...
mongoose.connect(`mongodb+srv://${config.mongo_user}:${config.mongo_password}@cluster0.p4iym.mongodb.net/${config.mongo_db_name}?retryWrites=true&w=majority`, {useNewUrlParser: true})
...
app.get('/', (req, res) => res.send('<h1>Hello! I am from the backend!</h1>'));

// Add a new celeb to DB
app.get("/addCeleb", (req, res) => {
    console.log("posted new celebrity");
    const dbCeleb = new Celebrity({
        _id: new mongoose.Types.ObjectId,
        name: "Sam",
        netWorth: 530
    });
    dbCeleb.save().then(result => {
      console.log("saved new celeb");
        res.send(result);
    }).catch(err => res.send(err));
}); // end of add product to DB

app.get("/allCelebs", (req, res) => {
  console.log("You asked for all the celebrities");
